      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card p-6">
          <div
            ...
            className="flex min-h-[260px] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 p-4 text-center transition hover:border-brand-400 dark:border-slate-700"
          >
            {preview ? (
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div>
          <p className="font-semibold">
            Image quality: {q.passed ? "passed" : "flagged"}
          </p>
      ...
      {result.prediction ? (
        <div>
          <div className="mb-1 flex items-baseline justify-between">
            ...
            {result.prediction.backbone && (
              <span className="text-xs text-slate-400">{result.prediction.backbone}</span>
            )}
          </div>
        ...
        <p className="rounded-lg bg-slate-100 p-3 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          No trained model loaded — showing preprocessing only. Train a model to get
          classification + Grad-CAM here.
        </p>
      )}
...
function Figure({ title, src }: { title: string; src: string }) {
  return (
    <div>
      ...
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={src} alt={title} className="w-full rounded-lg border border-slate-200 dark:border-slate-800" />
    </div>
  );
}
